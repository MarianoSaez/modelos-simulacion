...
class Calentador:
    def __init__(self, mass, constant, t_init, t_final, resistor, thickness, radius, height, thermic_conductivity, _voltage=220, _env_temp=15) -> None:
        self.mass = mass
    ...
    def calculate_temp(self, t) -> float:
        ...
        requerimientos
        """
        P = self._voltage**2 / self.resistor # Watts
        Q = P * STEP # Joules
        DELTA_T = Q / (self.mass * self.constant) # Grados C
        T_IDEAL = DELTA_T + self.t_init

        # Perdidas
        Q_PERDIDO = STEP * ( self.k * self.area * (T_IDEAL - self._env_temp) ) / self.thickness
        Q_REAL = Q - Q_PERDIDO
